...
export class PSDACalculator {
  ...
  constructor(inputs) {
    ...
    // Work/metabolism
    this.Work_Output_W      = 0;
    this.Total_Metabolic_Rate = 0;
  ...
  GetInitialResult() {
    if (this.NewStartData == null) {
      ...
      // Initial temperature distribution across 6 segments × 104 radial nodes
      // Loaded from hardcoded NEWSTART data (originally read from file)
      const tempData = [
      ...
      ]; //Lines 10-111 of NEWSTART
      for (let row = 0; row < tempData.length; row++)
        for (let col = 0; col < 6; col++)
          this.T[col][row][0] = tempData[row][col];
    }
  }

  temp = [-.295700E-01,-.834749E+00,-.334682E+00,-.638274E-01,-.135299E+01,-.507884E-01]; //Line 112 of NEWSTART
  for (let i = 0; i < temp.length; i++){
    this.STORAG[i] = temp[i];
  }

  temp = [0.000000E+00]; //Line 113 of NEWSTART
  this.total_sweat_loss_kg = temp[0];
  ...
  OUTPUT(){
      ...
      //    return to main program without output
      if (!this.Is_Write_Output){
          return;
      }
      else{
          console.log("Tried to Write Output in the Javascript Version");
      }
      return;
      
      /* -------------------------------------------------------------------------------- */
      /* There is a large output system here for writing the output to a file
      /* As of right now we are not including it in the JS file this is subject to change
      /* It also populates ModelSteps[] but in the c# version it is never used
      /* -------------------------------------------------------------------------------- */
